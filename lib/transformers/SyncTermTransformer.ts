...
export interface ISyncTermTransformer {
  transformRDFTermUnsafe: (term: RDF.Term) => E.Term;
  transformLiteral: (lit: RDF.Literal) => E.Literal<any>;
}

export class SyncTermTransformer implements ISyncTermTransformer {
  public constructor(protected readonly openWorldType: ISyncSuperTypeProvider) {
  }
  ...
  public transformLiteral(lit: RDF.Literal): E.Literal<any> {
    ...
    if (isSubTypeOfSync(dataType, TypeURL.XSD_STRING, this.openWorldType)) {
      return new E.StringLiteral(lit.value, dataType);
    }
    if (isSubTypeOfSync(dataType, DT.RDF_LANG_STRING, this.openWorldType)) {
      return new E.LangStringLiteral(lit.value, lit.language);
    }
    if (isSubTypeOfSync(dataType, DT.XSD_DATE_TIME, this.openWorldType)) {
      ...
      if (Number.isNaN(dateVal.getTime())) {
        return new E.SyncNonLexicalLiteral(undefined, dataType, this.openWorldType, lit.value);
      }
      return new E.DateTimeLiteral(new Date(lit.value), lit.value, dataType);
    }
    if (isSubTypeOfSync(dataType, DT.XSD_BOOLEAN, this.openWorldType)) {
    ...
    if (isSubTypeOfSync(dataType, DT.XSD_DECIMAL, this.openWorldType)) {
      const intVal: number = P.parseXSDDecimal(lit.value);
      if (intVal === undefined) {
        return new E.SyncNonLexicalLiteral(undefined, dataType, this.openWorldType, lit.value);
      }
      if (isSubTypeOfSync(dataType, DT.XSD_INTEGER, this.openWorldType)) {
    ...
    const isFloat = isSubTypeOfSync(dataType, DT.XSD_FLOAT, this.openWorldType);
    const isDouble = isSubTypeOfSync(dataType, DT.XSD_DOUBLE, this.openWorldType);
    if (isFloat || isDouble) {
